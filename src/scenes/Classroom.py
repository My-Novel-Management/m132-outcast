...
def this_rule(w: World):
    ...
    return w.scene('この高校のルール',
            ...
            nori.talk("キケロのことを古いという人もいるんだけど、$meは彼が書き残したことって今の色々な思想の根幹にあるんじゃないかと思うんだ",
                "例えば多くが法治国家な訳じゃない？",
                "法律には義務と責任が生じるわ",
                "この本の中で語られた『中間義務』と『完全義務』については理解できた？"),
...
def evaluation(w: World):
    ...
    return w.scene("成績評価",
            ...
            ako.do("ゲートチェックを受けた時点で$sivaにはデータが送られ、その時点で既に終わっているはずなのに、",
                "こうして目視でそのデータとの整合性を取ろうとするのはまだまだ古い人間が上に座っているのだなとしか思えなかった"),
            mizu.talk("おはようございます", "&"),
            ...
            ako.do("一部ざわついたものの、クラス長の$full_misoraが小さく咳払いをすると一瞬で止み、",
                ...
                "点数と判定、それぞれの名前が列挙される",
                "トップは九十二点の$misoraだ", "時点で九十点の$noriと続く",
            ...
            mizu.do("担任は敢えて事務的な口調でそう強調した",
                "入学式当日にも釘を差すようにそう口にしたが校長や管理部長からよほどきつく言われているのだろう"),
            ako.do("$sivaとは『$full_siva』つまり『$jpn_siva』である",
                "従来は$AIによる学習評価と言ってもただデータの分析方法をコンピュータに頼ったものだったり、",
...
def outcast(w: World):
    ...
    return w.scene("ハミダシモノ",
            ...
            mizu.come(),
            mizu.do("タイミング良く入ってきた$mizu先生は珍しく渋い表情で教壇に立つと生徒が静まり返るのを待ってから、こう口を開いた"),
            mizu.talk("みんな確認したと思いますが、$full_noriさんの件についてイジメがあったという報告が上がっていることは前に話した通りですが、",
            ...
            mizu.talk("$sivaが提案するのは輪番制のハミダシモノを選出するということです",
                ...
                "それで今この時間から最初のハミダシモノが決まるまで、緊急学級会議を行います",
                "終了時刻は未定", "つまり誰かがハミダシモノに立候補するまで、ということですね",
                "分かりましたか？"),
            ...
            ako.do("立候補という言葉が手を挙げることすら難しくして、しばらく教壇に立つ$mizuと何も口にしようとしない生徒との睨み合いになった",
                ...
                "そんな貧乏くじは誰も引きたがらない"),
            ako.do("$mizuが腕組みをしたまま椅子に腰を下ろしてから、一時間が経過することになった"),
            mizu.talk("誰もいない？"),
            mizu.do("$Sの声に誰も反応せず、それどころか視線を合わせることすらしない", "&"),
            ...
            ako.talk("どの責任について？", "法的？　それとも社会的責任のことかしら？"),
...
def alone_regular(w: World):
    ...
    return w.scene("孤独な正規者",
            ...
            misora.talk("$ln_noriさんに対する責任があるのではないか、というようなことは言ったかも知れませんね",
                ...
                "それなのに$ln_akoさんは特に何も感じていらっしゃらないようにお見受けするんだけど、どうなの？"),
