...
def protest(w: World):
    ...
    return w.scene("抗議",
            ...
            misora.come("$Sたちが戻ってくる"),
            noza.come(),
            noza.talk("どうして真面目に勉強してる$meたちが大きく評価下がって、ハミダシモノのアイツがトップなのよ",
                "先生も先生だわ",
                "みんながおかしいって言ってるのに、$AIによる判定は絶対ですって"),
            misora.talk("所詮はコンピュータのプログラムだから",
                "もとは人間が作ったものだし、それこそ誰かが細工でもすれば簡単に結果が変わるものなんでしょう？",
                "ちゃんとその辺りは$mizu先生も調査をしてくれると約束してくれましたし、",
                "$meたちは今までと同じように過ごすだけですよ"),
            misora.do("余裕なふりで座るものの、手にしたスマホを落としてしまう"),
            ako.do("$Sの足元に転がってきて、それを見る"),
            ako.do("ハンカチで包んで拾い上げてから、$misoraの机の上にそっと置いて"),
            ako.go("教室を出ていく"),
            )
...
def alone_regular(w: World):
    ...
    return w.scene("孤独な正規者",
            ...
            ako.be("電子黒板には大きく『自習』と書かれていた"),
            ako.do("生徒たちは好き勝手している"),
            misora.be("参考書を開いている"),
            ako.do("$misora以外の生徒は漫画を読んだり、ゲームで遊んだり、お菓子を持ち込んで食べたりと、まるで学級崩壊の有様"),
            ako.do("その有様に$mizuは頭を抱えて出て行ってしまった"),
            ako.do("立ち上がり、一人だけ教室を抜ける"),
            mizu.be(),
            ako.do("$mizuを呼び止める"),
            ako.talk("あの、先生", "お願いがあります"),
            ako.do("その日、$Sはハミダシモノをやめた"),
            )
...
def dropout_outcasts(w: World):
    ...
    return w.scene("ハミダシモノたちの退学",
            ...
            ako.be(),
            ako.do("一ヶ月後、十月の成績評価をもって前期の評価とされた"),
            ako.do("そこではハミダシモノになっているものは全員評価Ｆ判定となり、落第が決定"),
            ako.do("かろうじて成績がついた$Sだけが、後期まで継続となった"),
            )
...
def new_class(w: World):
    ...
    return w.scene("新しいクラス",
            ...
            nori.talk("あれ？　みんなは？"),
            nori.do("髪を短くしてさっぱりした様子の$S"),
            ako.do("彼女を見て、教室を一度見回す"),
            ako.talk("おはよう、$nori"),
            nori.talk("うん、おはよう。$k_ako", "ねえ、他のみんなは？"),
            ako.talk("みんなって……今は$meと$noriだけだよ", "もうハミダシモノはいないから"),
            nori.do("前の席に座り、鞄から教科書とノートを取り出した"),
            nori.talk("あの、休んでいる間に送ってもらった資料なんだけど、ここがね"),
            ako.do("教えてあげる$S"),
            )
